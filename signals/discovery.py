...
#: long before any query here runs — so the client library has to reduce it to
#: its leading segments when it sets the tag. `PATH_SEGMENTS` below is a
#: display bound on top of that, not a substitute for it.
PATH_LABELS = ("path", "route", "uri_path", "url_path", "endpoint")

#: How much of a path a target name carries. Matches the advice given for the
#: tag itself: three segments is enough to tell `/v1/media/thumbs` from
#: `/v1/media/uploads` and short enough to read on a chart.
PATH_SEGMENTS = 3
