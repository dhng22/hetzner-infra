...
Set a **Tunnel hostname** on the component. It goes through the same Cloudflare
tunnel your applications use, so nothing is published on a host port, no
firewall rule is opened, and it survives losing a node the way they do.

Three steps, and the Credentials tab gives you all three:

1. Add that hostname to your tunnel, pointed at `tcp://<name>_gateway:27017`.
   **Put a Cloudflare Access policy on it** — a service token suits a machine.
   That policy is the door.
2. Run `cloudflared access tcp --hostname <yours> --url 127.0.0.1:27017`
   alongside your application.
3. Connect to `127.0.0.1`. The hostname belongs to the helper, not your driver.

Behind that hostname is a proxy which asks every member, from inside, which one
is the primary and forwards to it. That is why the external string carries no
discovery of its own — `redis://…` with no sentinels,
`mongodb://…?directConnection=true` with no `replicaSet`. **A discovery protocol
cannot cross the cluster boundary**: sentinel answers `cache_redis-2` and a
replica set answers `docs_mongo-3:27017`, Swarm service names that resolve on
the overlay network and nowhere else.

So the address survives an election, a member added under read pressure, the set
moving onto its own machines, the master's copy being dropped, and losing a
node. **You are never asked to update a consumer.**

**Your client installs nothing but `cloudflared`** — no certificate, no
`tlsCAFile=`. The proxy holds the TLS session to the member, verifying this
component's own authority, and mongod's `requireTLS` is untouched. The cost is
one layer: an application inside the cluster has an unbroken session to the
member and an external one does not. That hop is encrypted by Cloudflare across
the internet and by the IPsec overlay inside the cluster.

Three more things worth knowing:

* **A few milliseconds per round trip**, out to Cloudflare and back. Pooled
  connections hide most of it; chatty code will feel it.
* **External reads come from the primary**, whatever Reads-may-go-to-secondaries
  says. That switch is a contract with applications inside the cluster.
* **At rest there are no workers**, so "one per node" is one, on the master —
  the same guarantee the connector gives your apps, growing the same way.
